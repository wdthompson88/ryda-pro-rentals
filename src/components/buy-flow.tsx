...
function FundStep({
  ...
  const [error, setError] = useState<string | null>(null);
  // Card AND ACH both go through Stripe Checkout — only the
  // payment_method_types parameter differs. We surface that via a
  // hint to the API so Stripe shows the appropriate method on its
  // hosted page.
  const stripePaths: FundingMethod[] = ["card", "ach"];
  const isStripePath = fundingMethod !== null && stripePaths.includes(fundingMethod);
  // Stripe paths bypass the confirm-checkbox because the Stripe
  // redirect is itself the user-action signal.
  ...
  // Stripe paths (card + ACH): hit the API to create a Checkout
  // session, then window.location to the Stripe-hosted page. On
  // return, /share-purchase/[id] reads the row and renders real status.
  // If the API isn't configured (no Stripe / no admin client) the
  // route surfaces a clear error and we leave the user on this step.
    ...
    <div className="space-y-8">
      ...
      {fundingMethod === "card" && (
        <div className="rounded-2xl border border-red bg-red/5 p-6">
          ...
          <p className="mt-3 text-sm text-ink-soft">
            We never see your card or bank details. Stripe holds the funds
            until your verification clears, then releases them to the LLC&apos;s
            escrow account. You&apos;ll come back here to a real-time tracker
      ...
      {fundingMethod && fundingMethod !== "card" && (
        <label className="flex cursor-pointer items-start gap-3 rounded-xl border border-rule bg-surface p-4 text-sm">
          <input
            type="checkbox"
