...
export async function POST(req: Request) {
  try {
    if (!(await isAllowed(`help-escalation:${clientIp(req)}`, RATE_LIMIT, RATE_WINDOW_MS))) {
...
function renderEscalationHtml({
  ...
  const conversationHtml =
    conversation && conversation.length > 0
      ? `<div style="margin-top:18px;">
          <div style="font-size:11px;text-transform:uppercase;letter-spacing:.15em;color:#514C47;margin-bottom:8px;">Conversation context</div>
  ...
  return `
    ...
    ${
      note
        ? `<div style="margin-top:18px;">
            <div style="font-size:11px;text-transform:uppercase;letter-spacing:.15em;color:#514C47;margin-bottom:6px;">Their note</div>
    ...
    ${
      trigger_message
        ? `<div style="margin-top:18px;">
            <div style="font-size:11px;text-transform:uppercase;letter-spacing:.15em;color:#514C47;margin-bottom:6px;">Trigger phrase</div>
