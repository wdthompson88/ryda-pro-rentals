...
// /bookings/[id]/checkin — pickup-handover flow.
//
// Per audit (May 2026): the previous 22-line stub had a hardcoded
// "Ferrari 296 GTB" caption regardless of which booking the member
// landed on. This rewrite fetches the real booking row from
// Supabase, derives the asset display label from it, and wires the
// HandoverFlow's onSubmit to POST /api/bookings/[id]/handover.
//
// Auth: the API endpoint enforces ownership (caller must be the
// booking's user_id). RLS on bookings already limits the read here
// to the booking's owner + co-owners on the same LLC.
...
export default function CheckinPage() {
  ...
  // Resolve display label from the real asset metadata.
  ...
  // Wrong-status guardrail. The API also enforces this server-side
  // but a friendly client-side check avoids the user hitting a
  // confusing 409 deep into the flow.
    ...
    return (
      <>
        ...
        <div className="mx-auto max-w-2xl px-6 py-16">
          ...
          <h1 className="mt-2 font-display text-3xl text-ink">
            This booking isn&apos;t ready for pickup.
          </h1>
          <p className="mt-3 text-sm text-ink-soft">
            Current status:{" "}
            <span className="font-medium text-ink">{booking.status}</span>.
            Pickup is only available once the booking is{" "}
            <span className="font-medium text-ink">confirmed</span> by ops.
          </p>
          <Link
            href="/bookings"
            className="mt-6 inline-flex h-12 items-center justify-center rounded-full bg-ink px-7 text-sm font-medium text-cream hover:bg-red"
