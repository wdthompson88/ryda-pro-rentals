...
// Member-side owner dashboard for one car. Mirrors the my-boats
// dashboard structure — real ownership/LLC/bookings/payments wired
// through /api/account/my-asset, with the red accent + miles-not-nm +
// "Vehicle" vocabulary that the cars vertical uses.
...
export function MyCarDashboard({ vehicle }: { vehicle: Vehicle }) {
  const { state, refresh } = useMyAsset(`car:${vehicle.symbol}`);

  if (state.status === "loading") return <Skeleton vehicle={vehicle} />;
  ...
  return <Loaded vehicle={vehicle} data={state.data} />;
}

function Loaded({ vehicle: v, data }: { vehicle: Vehicle; data: MyAssetData }) {
  ...
  const yourDays = callerShares * v.daysPerYear;
  const yourMiles = callerShares * v.milesPerYear;
  const yourAnnualCost = callerShares * v.annualOpCost;
    ...
    <>
          ...
          <div className="mt-6 grid grid-cols-1 gap-8 lg:grid-cols-12">
            ...
            <div className="lg:col-span-5">
              ...
              <div className="mt-6 grid grid-cols-2 gap-4 text-sm">
                <Stat
                  label="Your shares"
                  value={`${callerShares} of ${v.shares}`}
                />
                <Stat
                  label="LLC status"
                ...
                />
                <Stat
                  label="Miles allowance (yr)"
                  value={yourMiles.toLocaleString()}
                />
                <Stat
                  label="Planned reservations"
                  value={`${plannedActive} of ${BOOKING_POLICY.planned.activeLimitPerShare * callerShares} active`}
                />
                <Stat
                  label="Short-notice"
                  value="Open · book up to 7 days out"
                />
              </div>
          ...
          <ul className="mt-4 divide-y divide-rule rounded-xl border border-rule bg-surface">
            ...
            {buildActivity(data).length === 0 && (
              <li className="px-5 py-8 text-center text-sm text-mute">
                No activity yet on this vehicle.
              </li>
            )}
          </ul>
        </div>
      ...
      <Section id="costs" title="Costs" alt>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          ...
          <BigStat
            label="Effective $/day"
            value={formatUSD(v.effectiveDailyCost)}
            sub={`${v.daysPerYear} days/yr usage`}
          />
        </div>
        <div className="mt-10">
          ...
          <ul className="mt-4 space-y-2 text-sm">
            <Line
              ...
              value={formatUSD(v.pricePerShare * callerShares)}
              note={
            ...
            <Line
              ...
              value={formatUSD(Math.round(yourAnnualCost * 0.6))}
              note="Billed quarterly"
            />
            <Line
              label="Insurance"
              value={formatUSD(Math.round(yourAnnualCost * 0.27))}
              note="Annual"
            />
            <Line
              label="Maintenance reserve"
              value={formatUSD(Math.round(yourAnnualCost * 0.13))}
              note="Held at LLC"
            />
            <Line
              label="Total annual"
      ...
      <Section id="bookings" title="Bookings">
        <div className="mb-6 flex items-end justify-between">
          <p className="text-sm text-ink-soft">Vehicle calendar</p>
          <Link
            href="/bookings/new"
        ...
        <AssetCalendar vehicleSymbol={v.symbol} vertical="cars" />
        <div className="mt-6">
        ...
        <ul className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          {data.llc ? (
            <>
              ...
              />
              <Doc
                title={`Vehicle Title (${data.llc.name})`}
                date={data.llc.formation_date ?? "Pending"}
                size="—"
              />
              {data.llc.ein && (
      ...
      <Section id="status" title="Vehicle Status" alt>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          <BigStat
            ...
            value="RYDA Miami"
            sub="Storage · climate-controlled"
          />
          <BigStat
            label="Miles allowance"
            value={`${yourMiles.toLocaleString()} mi`}
            sub={`${callerShares} share${callerShares === 1 ? "" : "s"} · ${v.milesPerYear.toLocaleString()}/share`}
          />
          <BigStat
            label="Days allowance"
            value={`${yourDays} days`}
            sub={`${v.daysPerYear}/share`}
          />
        </div>
        <p className="mt-6 text-xs text-mute">
          Live telemetry (location, odometer, last-service distance) is on
      ...
      {isLead && (
        ...
        >
          ...
          <p className="mb-6 max-w-2xl text-sm text-ink-soft">
            Material decisions (booking rules, vehicle modifications,
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <SettingCard
              ...
              desc="Fuel grade, mileage cap, delivery defaults."
            />
            <SettingCard
            ...
            <SettingCard
              ...
              desc="Vehicle update to all co-owners (no vote)."
            />
            <SettingCard
            ...
            <SettingCard
              ...
              desc="Flag a vehicle issue or escalate a member conflict."
            />
          </div>
        </Section>
      )}
...
function Skeleton({ vehicle }: { vehicle: Vehicle }) {
  return (
    <>
      <SiteHeader />
      <section className="border-b border-rule">
    ...
    <>
      ...
      <section className="mx-auto max-w-2xl px-6 py-20 sm:px-10 sm:py-28">
        ...
        <p className="mt-3 text-sm text-ink-soft">
          The my-cars dashboard shows your shares, bookings, payments, and
          LLC documents for vehicles you actively co-own.
        </p>
        <div className="mt-6 flex gap-3">
          <Link
            href="/signin?next=/my-cars"
  ...
  return (
    <>
      ...
      <section className="mx-auto max-w-2xl px-6 py-20 sm:px-10 sm:py-28">
        ...
        <h1 className="mt-3 font-display text-3xl text-ink">
          You don&apos;t hold shares in this vehicle.
        </h1>
        <p className="mt-3 text-sm text-ink-soft">
          This page is only shown to active co-owners. Once you complete a
          share purchase the vehicle will appear in your dashboard.
        </p>
        <div className="mt-6 flex gap-3">
          <Link
            href="/portfolio"
...
function Card({
  ...
  return (
    <div className="rounded-2xl border border-rule bg-surface p-6">
      ...
      <Link
        href={cta[1]}
        className="mt-4 inline-block text-sm font-medium text-red hover:text-red-deep"
      >
        {cta[0]} →
      </Link>
    </div>
