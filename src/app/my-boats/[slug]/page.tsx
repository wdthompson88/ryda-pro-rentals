...
const OWNED = ["wajer-55s", "pershing-6x"];

export async function generateStaticParams() {
  return OWNED.map((s) => ({ slug: s.toLowerCase() }));
}
...
export default async function MyVehiclePage({
  ...
  const { slug } = await params;
  const v = getBoatBySlug(slug);
  if (!v || !OWNED.includes(v.slug)) notFound();

  const isLead = v.slug === "wajer-55s";
    ...
    <>
      ...
      <Section id="documents" title="Documents" alt>
        <ul className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          ...
          <Doc title="Boat Title (RYDA Wajer 55 S LLC)" date="Mar 12, 2026" size="148 KB" />
      ...
      <Section id="status" title="Boat Status" alt>
        ...
        <div className="mt-10">
          ...
          <ul className="mt-4 divide-y divide-rule rounded-xl border border-rule bg-surface">
            ...
            <Service date="Mar 30, 2026" what="Intake inspection" notes="Boat accepted into RYDA fleet. 1,840 mi at intake." />
          </ul>
        </div>
      </Section>
      ...
      {isLead && (
        <Section id="settings" title="Member proposals · Proposal Coordinator">
          ...
          <p className="mb-6 max-w-2xl text-sm text-ink-soft">
            ...
            members. Material decisions, booking rules, boat
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <SettingCard
              ...
              desc="Send a boat update to all co-owners' activity feed (informational, no vote required)."
            />
            <SettingCard
              title="Upload document for review"
              desc="Add a doc to the boat's Documents tab. Required-reading flag goes to a co-owner vote."
            />
            <SettingCard
              title="Open a service request"
              desc="Flag a boat issue to RYDA, or escalate a co-owner conflict for mediation."
            />
          </div>
        </Section>
      )}
...
function Card({ title, body, cta }: { title: string; body: string; cta: [string, string] }) {
  return (
    <div className="rounded-2xl border border-rule bg-surface p-6">
      ...
      <Link href={cta[1]} className="mt-4 inline-block text-sm font-medium text-marine hover:text-marine-deep">
        {cta[0]} →
      </Link>
    </div>
  );
